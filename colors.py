...
def iprint(string):
    print(f'{ColorCodes.BRIGHT}{ColorCodes.AQUA}[{ColorCodes.ORANGE}CobraNET{ColorCodes.AQUA}]{ColorCodes.RESET}' + ' ' + f'{ColorCodes.WHITE}{ColorCodes.BRIGHT}{string}{ColorCodes.BRIGHT}{ColorCodes.RESET} ')

def cinput(string, color=ColorCodes.CYAN, b=False, maxlen=None) -> str:
    try:
        if maxlen is None:
...
def rinput(color=ColorCodes.CYAN):
    try:
        r = input(f'{ColorCodes.BRIGHT}{ColorCodes.PURPLE}[{ColorCodes.AQUA}CobraNET{ColorCodes.PURPLE}]{ColorCodes.AQUA} > {ColorCodes.RESET}')
        return r
    except EOFError:
        print("\n")
        return None